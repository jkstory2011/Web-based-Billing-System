...
- 자동 발송 스위치: **시스템 전체 on/off** + **고객별 개별 오버라이드**
...
// 싱글턴 테이블 — 항상 정확히 한 행만 존재 (id: "default")
...
**활성화 여부 판정** — 청구서마다 다음 순서로 "이 청구서에 자동 발송이 켜져 있는가"를 계산한다:
1. 해당 청구서 고객의 `autoReminderOverride`가 `true`/`false`(non-null)면 그 값을 그대로 따른다
2. `null`(오버라이드 없음)이면 `SystemSettings.autoReminderEnabled` 값을 따른다

`SystemSettings.autoReminderEnabled`의 기본값은 `false`다 — 배포 직후 검증 없이 실제 고객에게 자동 메일이 나가는 것을 막기 위해, ADMIN이 설정 화면에서 명시적으로 켜야 전체 자동 발송이 시작된다.

**대상 선정 및 발송**
- 대상: `SENT` 상태이고 `dueDate`가 지난 모든 청구서 중 위 기준으로 "활성화"된 것만
...
- 기존 수동 엔드포인트(`POST /admin/invoices/:id/remind`)는 활성화 여부와 무관하게 항상 동작한다 — on/off 스위치는 자동 발송에만 적용되고 수동 발송에는 영향을 주지 않는다
- 기존 수동 엔드포인트와 내부 발송 로직을 공유하도록 `InvoiceReminderService.sendReminder(invoiceId, stage, triggeredBy, adminUserId?)`로 리팩터링한다
...
- `GET /admin/settings` (신규, `ADMIN`만)
  - `{ autoReminderEnabled: boolean }` 반환 (없으면 최초 조회 시 `SystemSettings` 싱글턴 행을 `autoReminderEnabled: false`로 생성)
- `PATCH /admin/settings` (신규, `ADMIN`만)
  - Body: `{ autoReminderEnabled: boolean }`
- `PATCH /admin/customers/:id/auto-reminder-override` (신규, `ADMIN`만)
  - Body: `{ autoReminderOverride: boolean | null }` (`null`이면 오버라이드 해제 → 시스템 설정을 따름)
...
- **고객 상세 페이지** (`features/customers/customer-detail-page.tsx`)
  ...
  - "자동 알림 발송" 섹션 (ADMIN만 편집 가능): 3-state 선택(시스템 설정 따름 / 항상 켜짐 / 항상 꺼짐), 현재 시스템 전체 설정값도 함께 보여줘서 "따름"을 선택했을 때 실제로 어떻게 동작하는지 알 수 있게 함
...
- **신규 페이지** `/settings` (`features/settings/settings-page.tsx`, ADMIN만 접근 — 메뉴도 ADMIN에게만 노출)
  - "연체 자동 알림 발송" 토글 하나 (현재 값 표시 + 켜기/끄기), 설명 문구로 "켜면 매일 09:00에 연체 청구서에 자동으로 독촉 메일이 발송됩니다" 등 명시
...
- 백엔드: 각 서비스(`collection-reminder` 로직 확장, `collection-notes.service`, `customers.service`의 담당자 배정, `collections-aging.service`, `settings.service`의 전체 on/off + 고객 오버라이드 조합에 따른 활성화 판정)에 대한 Jest 단위 테스트 + 컨트롤러 e2e 유사 테스트(기존 `*.service.spec.ts` 패턴)
...
Prisma 마이그레이션 1개로 `ReminderStage`/`ReminderTrigger` enum, `CollectionNote`, `CollectionReminder`, `SystemSettings` 테이블, `Customer.collectionOwnerId`/`autoReminderOverride` 컬럼을 추가한다. 기존 데이터에 영향 없는 추가적(additive) 변경이므로 다운타임 없이 적용 가능하다. `GET /admin/settings`가 최초 호출 시 `SystemSettings` 싱글턴 행을 없으면 생성하므로 별도 시드 스크립트는 필요 없다. `apps/api/package.json`에 `@nestjs/schedule` 의존성을 추가한다.
